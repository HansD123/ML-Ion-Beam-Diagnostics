...
csv_file_path_aluminium = "deposit_tables/al_proton_energies.csv"
csv_file_path_scintillator = "deposit_tables/sc_proton_energies.csv"


def create_al_proton_spline(filepath: str) -> None:
    """
    Reads the aluminum energies csv table downloaded from NIST PSTAR website and fits the table to
    ...
    with open("pickles/al_proton_spline.pkl", "wb") as f:
        dill.dump(spline_al, f)


def create_sc_proton_spline(filepath: str) -> None:
    """
    Reads the scintillator energies csv table downloaded from NIST PSTAR website and fits the table to
    ...
    with open("pickles/sc_proton_spline.pkl", "wb") as f:
        dill.dump(spline_al, f)
...
def create_al_2d_proton_spline(energy_range: Tuple[float, float], thickness_range: Tuple[float, float]) -> None:
    ...
    """
    with open("pickles/al_proton_spline.pkl", "rb") as f:
    ...
    def calc_stopping_aluminium(
        ...
        """
        Models the protons path through the material as a series of steps to determine the energy remaining
        """
        h = thickness / steps
        i = 0
        # energy_lost = 0
    ...
    with open("pickles/al_proton_remaining_spline.pkl", "wb") as f:
...
def create_sc_proton_deposited_spline(
    ...
    """
    with open("pickles/sc_proton_spline.pkl", "rb") as f:
    ...
    with open("pickles/sc_proton_deposited_spline.pkl", "wb") as f:
        dill.dump(sc_energy_deposited_spline, f)


create_al_proton_spline(csv_file_path_aluminium)
create_sc_proton_spline(csv_file_path_scintillator)
create_al_2d_proton_spline((0, 5), (-5, 0))
create_sc_proton_deposited_spline((0, 5), 2e-3)
